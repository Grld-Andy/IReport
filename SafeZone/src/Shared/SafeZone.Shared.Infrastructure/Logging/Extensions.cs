...
using SafeZone.Shared.Infrastructure.Logging.Decorators;
using SafeZone.Shared.Infrastructure.Logging.Options;
using Serilog;
using Serilog.Events;
using Serilog.Filters;

namespace SafeZone.Shared.Infrastructure.Logging;

public static class Extensions
{
    private const string ConsoleOutputSafeZone = "{Timestamp:HH:mm:ss} [{Level:u3}] {Message}{NewLine}{Exception}";
    private const string FileOutputSafeZone = "{Timestamp:HH:mm:ss} [{Level:u3}] ({SourceContext}.{Method}) {Message}{NewLine}{Exception}";
    ...
    private static void Configure(LoggerConfiguration loggerConfiguration, LoggerOptions options)
    {
        ...
        {
            loggerConfiguration.WriteTo.Console(outputTemplate: ConsoleOutputSafeZone);
        }
        ...
        if (fileOptions.Enabled)
        {
            ...
            loggerConfiguration.WriteTo.File(path, rollingInterval: interval, outputTemplate: FileOutputSafeZone);
        }
