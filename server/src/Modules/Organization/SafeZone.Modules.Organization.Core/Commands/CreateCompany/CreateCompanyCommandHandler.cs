    ...
    ) : ICommandHandler<CreateCompanyCommand>
{
    ...
    public async Task HandleAsync(CreateCompanyCommand command, CancellationToken cancellationToken = default)
    {
        ...
        _ = messageBroker.PublishAsync(new CompanyRegisteredEvent(companyId, companyDto.CompanyName, result.Extension, companyDto.AdminName, companyDto.Email, companyDto.PhoneNumber, companyDto.PaymentRef, result.Url), cancellationToken);
